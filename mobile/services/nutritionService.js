import { USDA_API_KEY, isUsdaApiKeyConfigured } from '../config/nutritionApi';
...
const searchFoodsFromMetaboLiftDb = async queryText => {
  ...
  return rankFoods(dedupeFoods(mapped));
};
...
const getFoodDetailsFromUsda = async fdcId => {
  ...
  const payload = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(payload?.error?.message || payload?.message || 'USDA detay istegi basarisiz.');
  }
...
export const nutritionService = {
  getSetupState: () => ({
    provider: 'USDA',
    hasApiKey: isUsdaApiKeyConfigured(),
  }),

  getFallbackFoods: () => FALLBACK_FOODS,
  ...
  getInitialFoods: async () => {
    ...
    return FALLBACK_FOODS.slice(0, DEFAULT_DB_FOOD_LIMIT);
  },
  ...
  searchFoods: async query => {
    ...
    if (!isUsdaApiKeyConfigured()) {
      const fallbackMatches = FALLBACK_FOODS.filter(item =>
        item.name.toLowerCase().includes(normalized.toLowerCase())
      );
      return rankFoods(dedupeFoods([...localApproved, ...fallbackMatches]));
    }

    const results = await searchFoodsFromUsda(normalized);
    const merged = rankFoods(dedupeFoods([...localApproved, ...results]));
    if (merged.length > 0) return merged;

    return FALLBACK_FOODS.filter(item =>
      item.name.toLowerCase().includes(normalized.toLowerCase())
    );
  },
