...
const AdminFoodDatabaseScreen = ({ navigation }) => {
  ...
  React.useEffect(() => {
    ...
    try {
      unsubscribe = foodSubmissionService.subscribeApprovedSubmissions(
        ...
        error => {
          Alert.alert('Hata', error.message || 'Besin veritabani yuklenemedi.');
          setLoading(false);
          setRefreshing(false);
        }
      );
  ...
  const handleManualRefresh = async () => {
    setRefreshing(true);
    try {
      const rows = await foodSubmissionService.getApprovedSubmissions();
      setItems(rows);
    } catch (error) {
  ...
  const saveDetail = async () => {
    ...
    try {
      ...
      Alert.alert('Hata', error.message || 'Besin guncellenemedi.');
    } finally {
  ...
  const handleDelete = item => {
    Alert.alert(
      'Kaydi sil',
      `"${item.name}" kaydini silmek istedigine emin misin?`,
      [
        { text: 'Vazgec', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert('Hata', error.message || 'Besin silinemedi.');
            } finally {
              setDeletingId('');
            }
          },
    ...
    <SafeAreaView edges={['top']} style={styles.safeArea}>
      ...
      >
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Besin Veritabani</Text>
        </View>
        <Text style={styles.subtitle}>Onayli tum besinleri ara, duzenle veya sil.</Text>

        <TextInput
          style={styles.input}
          placeholder="Besin ara (ad, marka, porsiyon)"
          value={search}
        ...
        {filteredItems.length === 0 ? (
          <View style={styles.card}>
            <Text style={styles.emptyText}>Aramaya uygun besin yok.</Text>
          </View>
        ) : (
          filteredItems.map(item => (
            <View key={item.id} style={styles.card}>
              <Text style={styles.itemTitle}>{item.name}</Text>
              <Text style={styles.itemMeta}>Marka: {item.brandName || '-'}</Text>
              <Text style={styles.itemMeta}>Porsiyon: {item.serving || '-'}</Text>
              <Text style={styles.itemMeta}>Son guncelleme: {formatDate(item.updatedAtMs || item.createdAtMs)}</Text>
              ...
              <View style={styles.row}>
                ...
                >
                  <Text style={styles.actionText}>{deletingId === item.id ? 'Siliniyor...' : 'Sil'}</Text>
                </TouchableOpacity>
              </View>
            </View>
          ))
        ...
        <SafeAreaView style={styles.safeArea}>
          <ScrollView contentContainerStyle={styles.container}>
            <View style={styles.header}>
              ...
              <Text style={styles.title}>Besin Duzenle</Text>
            </View>
            {detailForm ? (
              <View style={styles.card}>
                ...
                <TouchableOpacity style={styles.saveButton} onPress={saveDetail} disabled={savingDetail}>
                  <Text style={styles.saveButtonText}>{savingDetail ? 'Kaydediliyor...' : 'Kaydet'}</Text>
                </TouchableOpacity>
              </View>
            ) : null}
          </ScrollView>
